...
tbl_contrat_join <- tbl_contrat |>
  ...
  ungroup() |>
  # calcul de l'âge
  mutate(age = 2022 - as.numeric(substr(date_naissance, -4, -1)))
...
tbl_contrat_join_2022 <- tbl_contrat_join |>
  filter(year(date_fin_contrat) >= 2022 &
           year(date_debut_contrat) <= 2022) |>
  # sinon date_diff ?
  mutate(
    duree = difftime(date_fin_contrat, date_debut_contrat, units = "days"),
    duree = as.numeric(duree) + 1
  ) |>
  group_by(id_assure, is_boeth_assure) |>
  summarise(n_jours_couverts_ontrats = sum(duree),
            .groups = "drop") |>
  mutate(duree_inactivite = 365 - min(n_jours_couverts_ontrats, 365))

# on collect par age
df_age_jours_couverts <- tbl_contrat_join_2022 |>
  count(is_boeth_assure, age, duree_inactivite) |>
  collect() |>
  mutate(groupe_age = cut(age, breaks = c(0, 25, 50, 100))) |>
...
df_contrat_diff_sur_boeth <- tbl_contrat_join |>
  group_by(id_assure) |>
  summarise(
    is_boeth_max = max(boeth),
    is_boeth_min = min(boeth),
    .groups = "drop"
  ) |>
